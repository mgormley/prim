package edu.jhu.prim.iter;

public class ShortIncrIter implements ShortIter {

    private final short start;
    private final short end;
    private final short incr;   
    private short cur;
        
    /** 
     * Constructs a new IntIter which ranges from start (inclusive) to end (exclusive) by steps of incr.
     * 
     * @param start The first value (inclusive) over which to iterate.
     * @param end The last value (exclusive) over which to iterate.
     * @param incr The increment value.
     */
    public ShortIncrIter(short start, short end, short incr) {
        this.start = start;
        this.end = end;
        this.incr = incr;
        this.cur = start;
    }

    /** Standard constructor which increments by 1 from 0 to end. */
    public ShortIncrIter(short end) {
        this(0, end, 1);
    }
    
    @Override
    public short next() {
        if (!hasNext()) {
            throw new IllegalStateException();
        }
        short tmp = cur;
        cur += incr;
        return tmp;
    }

    @Override
    public boolean hasNext() {
        return cur < end;
    }

    @Override
    public void reset() {
        this.cur = start;
    }
    
}
